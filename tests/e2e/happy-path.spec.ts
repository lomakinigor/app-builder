...
test('E2E-001 — new project → idea → research → spec → architecture → prompt loop → review', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'AI Product Studio' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Идея' })).toBeVisible()
  ...
  await page.getByRole('button', { name: /Сохранить и перейти к исследованию/ }).click()

  // ── 4. Research page ──────────────────────────────────────────────────────────

  await expect(page).toHaveURL(/\/research/)
  await expect(page.getByRole('heading', { name: 'Исследование' })).toBeVisible()
  ...
  // "Перейти к спецификации" may appear in both PageHeader action and inside the Card
  await page.getByRole('button', { name: /Перейти к спецификации/ }).first().click()

  // ── 5. Spec page ──────────────────────────────────────────────────────────────

  await expect(page).toHaveURL(/\/spec/)
  await expect(page.getByRole('heading', { name: 'Спецификация' })).toBeVisible()

  await page.getByRole('button', { name: 'Сгенерировать спецификацию' }).click()
  ...
  // Bottom action bar
  await page.getByRole('button', { name: /Перейти к архитектуре/ }).click()

  // ── 6. Architecture page + persistence checkpoint ─────────────────────────────

  await expect(page).toHaveURL(/\/architecture/)
  await expect(page.getByRole('heading', { name: 'Архитектура' })).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Сгенерировать архитектуру' }).click()
  // Wait for mock arch (~1 000ms) — "Сгенерировано" badge
  await expect(page.getByText('Сгенерировано').first()).toBeVisible({ timeout: 10_000 })

  await page.getByRole('button', { name: /Перейти к циклу промптов/ }).click()

  // ── 7. Prompt Loop ────────────────────────────────────────────────────────────

  await expect(page).toHaveURL(/\/prompt-loop/)
  await expect(page.getByRole('heading', { name: 'Цикл промптов' })).toBeVisible()

  await page.getByRole('button', { name: 'Сгенерировать первый промпт' }).click()
  // Wait for mock prompt (~800ms) — iteration card header
  await expect(page.getByText('Итерация 1')).toBeVisible({ timeout: 8_000 })
